	...
	$.fn.trackThis = function(settings) {
		...
		var init = function() {
				...
				if (settings.AltUA) {
					try {
						window._gaq = window._gaq || [];
						_gaq.push(['b._setAccount', settings.AltUA]);
				...
				$('a[href^="tel:"]').on("click", function(event) {
					...
					_gaq.push(['_trackEvent', 'Telephone', 'Clicked', theLabel], function() {
						return true;
					});
				});
				...
				$('a[href^="mailto:"]').on("click", function(event) {
					var theLabel = $(this).attr('href');
					_gaq.push(['_trackEvent', 'Email', 'Clicked', theLabel], function() {
						return true;
					});
				});
				...
				$('a[href^="#"]').on("click", function(event) {
					var theLabel = $(this).attr('href');
					_gaq.push(['_trackPageview', $(location).attr('pathname') + theLabel], function() {
						return true;
					});
				});
				...
				$('a[href*="://"]:not([href*="' + location.hostname.replace("www.", "") + '"])').on("click", function(event) {
					...
					_gaq.push(['_trackEvent', 'External Link', 'Clicked', theLabel], function() {
						return true;
					});
				});
